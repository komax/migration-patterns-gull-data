...
def normalize_dict_heatmap(heatmap, valuebins, lons, lats):
...
def normalize_array_heatmap(heatmap, valuebins, lons, lats):
	print('start normalizing')
	normalized_hm = np.zeros([lons, lats])
	factor = 10.0/len(valuebins)
	for i in range(lons):
...
def determine_bin_boundaries(values, nrbins):
	values.sort()
	bin_values= []
	length_remaining = len(values)
	bins_remaining = nrbins
...
def generate_heatmap(filein, fileout, nrlonbins = 200, nrlatbins = 400):
	...
	normalized_heatmap = normalize_dict_heatmap(heatmap_bins, bin_values, lon_bins, lat_bins)
	del heatmap_bins	
...
def generate_raw_heatmap_per_gull(filein, nrlonbins = 200, nrlatbins = 400):
	gull_data, min_lon, max_lon, min_lat, max_lat = read_data_by_gull(filein)
	...
	# The gullnames are the key of the dictionary
	for gull in gull_data:
		heatmap_bins = np.zeros([lon_bins, lat_bins])
		singleton_dic = { gull : gull_data[gull]}
		FillHeatmap(singleton_dic, heatmap_bins, min_lon, min_lat, lon_bin_size, lat_bin_size, lon_bins, lat_bins)
		fileout = "%s.pkl"%(gull.replace(' ','_'))
		with open(fileout, 'wb') as fh:
			pickle.dump([heatmap_bins, min_lon, max_lon, min_lat, max_lat, lon_bins, lat_bins], fh)


def generate_heatmap_for_gulls(gull_ids, fileout = None, nrlonbins = 200, nrlatbins = 400):
	with open("%s.pkl"%(gull_ids[0].replace(' ','_')), 'rb') as fh:
		[raw_heatmap, min_lon, max_lon, min_lat, max_lat, lon_bins, lat_bins] = pickle.load(fh)

	heatmap = np.zeros([lon_bins, lat_bins])
	for gull in gull_ids:
		filein = "%s.pkl"%(gull.replace(' ','_'))
