...
(function (global) {
    ...
    function Calendarmap(id, range /*[first year, last year]*/) {
        ...
        var width = 700, height = 105, cellSize = 12, // cell size
        week_days = ['Sun', 'Mon', 'Tue', 'Wed', 'Thu', 'Fri', 'Sat'], month = ['Jan', 'Feb', 'Mar', 'Apr', 'May', 'Jun', 'Jul', 'Aug', 'Sep', 'Oct', 'Nov', 'Dec'];
        var day = d3.time.format("%w"), week = d3.time.format("%U"), percent = d3.format(".1%"), format = d3.time.format("%Y%m%d"), parseDate = d3.time.format("%Y%m%d").parse;
    ...
    function visualizeCalendar(data, gullIDs) {
        ...
        var stopoverDays = this.stopoverDays = {}, stopoverGulls = this.stopoverGulls = {};
        ...
        var counts = $.map(stopoverDays, function (v) { return v; });
        ...
        this.rect //.filter(function(d) { return stopoverDays[d]; })
            ...
            $(this).tooltipster('content', formatNames(stopoverGulls[d] || [])
                .prependText('' + stops + (stops == 1 ? ' stop' : ' stops')));
        });
    }
    //------------------------------------------------------------------------------
    $.fn.prependText = function (text) {
        return this.each(function () { $(this).prepend(document.createTextNode(text)); });
    };
    //------------------------------------------------------------------------------
    function Range() {
        this.list = [];
    ...
    Range.prototype.merge = function merge(fudge) {
        ...
        var list = [], range = this.list.pop(), head = undefined;
...
//------------------------------------------------------------------------------
//# sourceMappingURL=calendarmap.js.map